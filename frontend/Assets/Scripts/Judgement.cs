...
public class Judgement : MonoBehaviour
{
    ...
    Queue<Note> noteLeftRot = new Queue<Note>();
    Queue<Note> noteRightRot = new Queue<Note>();

    int[] longNoteCheck = new int[4] { 0, 0, 0, 0 };

    int curruntTime = 0;
    /// <summary>
    /// Judgment timing adjusted by user
    /// </summary>
    ...
    public void Init()
    {
        ...
        foreach (var note in GameManager.Instance.sheets[GameManager.Instance.title].notes)
        {
            ...
            // Left rotate note
                ...
                noteLeftRot.Enqueue(note);
            // Right rotate note
            else if (note.line == 6)
                noteRightRot.Enqueue(note);
        }
        notes.Add(note1);
        notes.Add(note2);
        notes.Add(note3);
        notes.Add(note4);
        notes.Add(noteLeftRot);
        notes.Add(noteRightRot);
    ...
    IEnumerator IECheckMiss()
    {
        while (true)
        {
            ...
            for (int i = 0; i < notes.Count; i++)
            {
                ...
                if (note.type == (int)NoteType.Long)
                {
                    if (longNoteCheck[note.line - 1] == 0) // When the head is not judged
                    {
                        if (judgeTime < -miss)
                        {
