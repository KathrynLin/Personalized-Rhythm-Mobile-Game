...
from mug.data.convertor import save_osu_file, parse_osu_file,convert_osu_to_new_format
...
def startMapping(audioPath, audioTitle, audioArtist,
    ...
    try:
        ...
        with torch.no_grad():
            ...
            x_samples_ddim = model.model.decode(samples_ddim).cpu().numpy() # decode the latent space code into original space
            ...
            for i, x_sample in enumerate(progress.tqdm(x_samples_ddim, desc='Post process charts')):
                ...
                #file_name = f"{audioArtist} - {audioTitle} ({creator}) [{version}].osu".replace("/",
                 #                                                                               "")
                file_name = f"{audioTitle}"
                file_path = os.path.join(save_dir, file_name)
                #sheet_file_path = file_path.replace('.osu','.sheet')
                ...
                new_format_content = convert_osu_to_new_format(osu_content)

                converted_sheet_path = file_path+'.sheet'
                with open(converted_sheet_path,'w') as f:
                ...
                if uploaded_image_data is None:
                    shutil.copyfile("asset/bg.jpg", os.path.join(save_dir, f"{audioTitle}.jpg"))# Modifed by LINDA
                else: 
                    if uploaded_image_data.mode == 'RGBA':
                        uploaded_image_data = uploaded_image_data.convert('RGB')
                        print(uploaded_image_data.mode)
                    uploaded_image_data.save(os.path.join(save_dir, f"{audioTitle}.jpg"))
                ...
                previews.append(pic)
            # package
            output_osz_path = os.path.join(output_path, save_name + '.zip')
...
if __name__ == "__main__":
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                audioPath = gr.File(label="Audio file", info="drop audio here", type="file",
                                     file_types=['audio'])
                imagePath = gr.File(label="A cover for the song", info="Drop image here", type="file", file_types=['png', 'jpg', 'jpeg'])
                
            with gr.Column(scale=1):
                audioTitle = gr.Textbox(label="Audio title", lines=1)
                audioArtist = gr.Textbox(label="Audio artist", lines=1)
                imageTitle = gr.Textbox(label="Image Uploading Status", lines=1)
            ...
            def on_change_image(x):
                    print(x['name'])
                    global uploaded_image_data 
                    uploaded_image_data = None
                    uploaded_image_data = Image.open(x['name'])
                    if uploaded_image_data is not None:
                        image_title = 'Upload Success'
                    else:
                        image_title = 'Failure'
                    
                    return gr.update(value=image_title)
                    
            audioPath.change(on_change_audio, inputs=audioPath, outputs=[audioTitle, audioArtist],
                             preprocess=False, postprocess=False)
            imagePath.change(on_change_image, inputs=imagePath, outputs=[imageTitle], preprocess=False, postprocess=False)
            # gr.update(audioTitle,value=''),
            # gr.update(audioArtist,value='')
    ...
    webui.queue(10).launch(share=True, favicon_path='asset/logo.ico', inbrowser=True)